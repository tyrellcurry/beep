    ...
    <View style={styles.tabButtonsContainer}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabButtonsContent}>
        {["Open Now", "Nearby", "Safe Zones", "Public Service"].map((label) => (
          <TouchableOpacity key={label} style={[styles.tabButton, activeTab === label && styles.activeTabButton]} onPress={() => handlePress(label)}>
            <Text style={[styles.tabButtonText, activeTab === label && styles.activeTabButtonText]}>{label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    </View>
